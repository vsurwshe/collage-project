...
class Login extends CI_Controller
{
    ...
    function index ()
    {
        ...
        // checking if user is already login and redirect him.
        ...
        if ($this->input->post())
        {
            ...
            {
                // running login query, all parameters are passed in model.
                $query = $this->users_model->validate_login();

                // if query retruned user row.
                if ($query)
                {
    ...
    function logout ()
    {
        ...
        // redirect - no check is needed here.
        redirect('/login');
    }
