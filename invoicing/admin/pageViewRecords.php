				...
				<table>
					...
					<tr>
						<td align="center">
							Show records from
							<?php
								$arrFields=array('', 'clients', 'invoices', 'invoice_items');
								$arrFieldCaptions=array('All tables', 'Clients', 'Invoices', 'Invoice items');
...
<?php
	...
	$res=sql("select r.recID, r.memberID, g.name, r.tableName, r.dateAdded, r.dateUpdated, r.pkValue from membership_userrecords r left join membership_groups g on r.groupID=g.groupID $where $sortClause limit $start, ".$adminConfig['recordsPerPage']);
