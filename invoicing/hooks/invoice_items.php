<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'invoice_items' table. 
	...
	*/
	 ...
	 *     $data['invoice'], $data['item'], $data['unit_price'], $data['qty'], $data['tax'], $data['price']
	...
	*/
	...
	function invoice_items_after_insert($data, $memberInfo, &$args){
		...
		sql("update invoice_items set price='$price' where id='$id'", $eo);
		
		updateInvoiceTotals($data['invoice']);
		return TRUE;
	}
	 ...
	 * For this table, the array items are: 
	 *     $data['id'], $data['invoice'], $data['item'], $data['unit_price'], $data['qty'], $data['tax'], $data['price']
	...
	*/
	...
	function invoice_items_after_update($data, $memberInfo, &$args){
		return invoice_items_after_insert($data, $memberInfo, $args);
	}
