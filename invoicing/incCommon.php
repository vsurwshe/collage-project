<?php
	...
	*/
	...
	function getTableList(){
		$arrTables=array(
			'clients'=>array('Clients', '', 'table.gif'),
			'invoices'=>array('Invoices', '', 'table.gif'),
			'invoice_items'=>array('Invoice items', '', 'table.gif')
			);
	...
	function logInMember(){
		...
		if($_POST['signIn']!=''){
			if($_POST['username']!='' && $_POST['password']!=''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('invoicing_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('invoicing_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['invoicing_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['invoicing_rememberMe']);
	...
	#########################################################
	function parseCode($code, $isInsert=true){
		if($isInsert){
			$arrCodes=array(
				...
				'<%%creationdate%%>' => date('j/n/Y'),
				'<%%creationtime%%>' => date('h:i:s a'),
				'<%%creationdatetime%%>' => date('j/n/Y h:i:s a'),
			...
			$arrCodes=array(
				...
				'<%%editingdate%%>' => date('j/n/Y'),
				'<%%editingtime%%>' => date('h:i:s a'),
				'<%%editingdatetime%%>' => date('j/n/Y h:i:s a'),
